...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('nightcore')
        .setDescription('Apply nightcore filter to the music (faster with tremolo)'),
    ...
    async execute(interaction) {
        try {
            ...
            if (activeFilter === 'nightcore') {
                return interaction.editReply({ embeds: [errorEmbed('Nightcore filter is already active!')] });
            }
            
            // Apply the filter
            const success = await applyFilter(player, 'nightcore');
            
            if (success) {
                const successEmbed = createEmbed({
                    title: `Filter Applied`,
                    description: `Nightcore filter has been applied to the music. The music will play faster with a tremolo effect.`,
                    fields: [
                ...
                return interaction.editReply({ embeds: [errorEmbed('Failed to apply the nightcore filter. Please try again.')] });
            }
        } catch (error) {
